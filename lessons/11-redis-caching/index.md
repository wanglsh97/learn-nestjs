...
数据库查询已经正确，但热点列表和单条读取每次都访问数据库。本课引入旁路缓存（Cache-Aside）：读取先查缓存，未命中再查数据库并回填；写入成功后主动失效相关缓存键。配置 Redis 时使用共享缓存，未配置、连接失败或运行时操作失败时降级到带 TTL 的进程内缓存。

![旁路缓存读取与失效流程](diagram/cache-aside-flow.svg)

## 缓存是副本，不是事实来源

数据库仍是事实来源（Source of Truth）。缓存可以删除和重建，因此不应保存唯一业务状态。旁路缓存的读取路径是：
...
首次请求日志显示 `Cache miss`，第二次相同请求显示 `Cache hit`。日志只输出缓存键的资源前缀，不记录搜索条件或用户输入。

## 缓存键必须包含所有影响结果的上下文

单条缓存键使用 `note:<id>:<role>:<userId>`，列表缓存键使用角色、用户 ID 和规范化后的查询 DTO。角色和用户不能省略，否则普通用户可能命中管理员或其他用户缓存，造成越权数据泄漏。把 Note ID 放在前面，也让写操作能用 `note:<id>:` 一次失效管理员与所有者的全部视图。

缓存键设计还要考虑 schema 版本、租户、语言等上下文。不要直接把密码、令牌或长请求体写进缓存键。
...
热点缓存键同时过期会造成缓存击穿。生产可使用随机抖动、请求合并或互斥锁；大面积过期和 Redis 故障造成的缓存雪崩还需要限流与数据库容量保护。
...
登录后连续两次请求相同列表，终端依次出现未命中和命中。创建或更新 Note 后再查询，会再次出现未命中。
...
启动日志不再出现内存降级警告。可以执行 `docker compose exec redis redis-cli TTL '<key>'` 观察剩余 TTL；缓存键的具体值可通过 Redis CLI 扫描，仅限本地学习环境。

## 工程取舍与易错点

- 永远先完成数据库写入，再失效缓存；反过来会让并发读取重新填入旧值。
- 缓存反序列化得到普通对象，不能依赖实体（Entity）实例方法或 `Date` 实例语义。
- 多实例不能把内存降级当成共享一致缓存。
