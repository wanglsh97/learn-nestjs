...
NestJS 把这件事交给 IoC 容器：模块声明当前上下文里有哪些 Provider 和 Controller，容器读取构造函数元数据，按依赖关系创建实例。
...
- `imports`：当前模块要使用哪些模块公开的 Provider；
- `controllers`：哪些类负责接收传入请求；
- `providers`：哪些类或工厂由当前模块的容器管理；
- `exports`：哪些 Provider 可以被导入本模块的其他模块使用。

不要为了省事把所有 Provider 都放进 `AppModule`。根模块适合组合边界，功能模块负责封装业务能力。
...
真实项目还可以使用字符串或 `Symbol` Token，并通过 `useValue`、`useFactory`、`useExisting` 替换来源。这适合配置、第三方客户端或同一能力的多种实现。本课使用类 Token，因为依赖关系最直接。
...
如果删除 `exports: [ClockService]`，即使 `NotesModule` 已经导入 `ClockModule`，启动时仍会出现 Nest 无法解析 `NotesService` 依赖的错误。Provider 默认只在声明它的模块上下文中可见。

模块导出的是 Token 对应的能力，不是把内部所有 Provider 自动变为公开 API。保持较小的导出面可以减少跨模块耦合。

## Provider 生命周期怎么选

Nest Provider 默认是单例作用域：一个应用上下文通常复用同一实例。本课的 `NotesService` 使用内存 `Map`，所以多次请求能看到之前创建的笔记。

另外两种作用域需要谨慎使用：

- request scope：每个请求创建一次，适合确实依赖请求上下文的对象；
- transient scope：每个注入点获得新实例，适合需要独立瞬时状态的轻量对象。

request scope 会沿注入链向消费方冒泡：单例 Provider 若依赖 request-scoped Provider，自身也必须按请求创建。transient 不会这样传播；它只为每个消费者提供独立实例，不改变消费者自身的作用域。两者都会增加实例创建成本，因此大多数无状态 Service、Repository 和客户端应保留默认单例。请求身份通常通过 Guard 或显式上下文传递，而不是把整条依赖链改成 request scope。
...
连续创建两条笔记，可以看到每次 `createdAt` 都来自同一个时钟能力，而 Controller 完全不知道时间如何产生。若真实项目需要固定时间、租户时区或外部时间源，只需替换 Token 对应的 Provider，`NotesService` 的调用方式不变。

本课 Demo 只保留源码和可直接运行的 HTTP 行为，不包含测试用例；第 13 课会再用自动化测试展示如何替换 Provider。
...
源码能导入一个类，不代表运行时容器能注入它。检查 Token 是否注册在当前模块，或是否由已导入模块明确导出。
...
本课 Demo 的最终依赖方向是单向的：根模块组合功能模块，`NotesModule` 使用 `ClockModule` 公开的时钟能力，业务 Service 不反向依赖 Controller 或根模块。
