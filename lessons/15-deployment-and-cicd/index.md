...
## 迁移（Migration）必须先兼容，再切换代码

Demo 为方便本地运行，在应用启动时执行 TypeORM 迁移。多个生产副本同时启动并抢跑迁移会带来锁和不确定性，生产应使用独立的一次性发布任务，并遵循扩展/收缩（expand/contract）模式：

1. 先发布向后兼容的结构变更，例如新增可空列或新表。
2. 再发布同时兼容新旧结构的应用，逐步回填数据。
3. 确认旧版本已退出后，才收紧约束或删除旧列。

这样滚动期间新旧版本可以同时访问数据库。破坏性迁移通常不能靠“把镜像回滚”撤销，因此需要单独的恢复方案和备份验证。

## 停机与健康信号决定流量边界

应用调用 `enableShutdownHooks()`。平台发送 `SIGTERM` 后，Nest 会执行生命周期钩子；BullMQ 工作进程和队列在 `onModuleDestroy` 中关闭。生产环境还需设置足够的终止宽限期（termination grace period），并先把实例标记为未就绪（unready），让负载均衡停止送入新请求，再等待进行中的请求和任务结束。

Compose 使用 `/api/health/ready` 做容器健康检查。Kubernetes 等平台应分别配置：

- 存活检查（liveness）→ `/api/health/live`，决定是否重启；
- 就绪检查（readiness）→ `/api/health/ready`，决定是否接收流量；
- 对慢启动应用可再用启动探针（startup probe），避免初始化期间被存活检查误杀。
...
可靠发布不是“CI 通过后立刻替换全部实例”。常见流程是：运行兼容迁移 → 部署少量新实例 → 就绪检查（readiness）通过 → 逐步增加流量 → 观察第 14 课的错误率、延迟和资源饱和度。
